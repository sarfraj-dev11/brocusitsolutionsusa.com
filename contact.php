...
<!-- PAGE HERO -->
<section class="page-hero" style="background:linear-gradient(160deg, rgba(30,10,80,.88) 0%, rgba(10,5,32,.80) 40%, rgba(5,5,8,.92) 100%),url('<?= asset('images/contact-hero-bg.jpg') ?>') center center / cover no-repeat;position:relative;overflow:hidden;">
  <div style="position:absolute;top:-100px;left:-100px;width:600px;height:600px;background:radial-gradient(circle,rgba(139,92,246,.3),transparent 65%);pointer-events:none;"></div>
  ...
  <div class="page-hero-inner" style="position:relative;z-index:2;">
    <span class="label"><i class="fas fa-headset" style="font-size:.7rem"></i> Contact Us</span>
...
<section style="padding:7rem 0;background:linear-gradient(135deg,#0D0829 0%,#130D35 50%,#0A0520 100%);position:relative;overflow:hidden;">
  <div style="position:absolute;top:-200px;right:-200px;width:600px;height:600px;background:radial-gradient(circle,rgba(96,165,250,.1),transparent 70%);pointer-events:none;"></div>
  <div style="max-width:1200px;margin:0 auto;padding:0 1.5rem;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,420px),1fr));gap:4rem;align-items:start;position:relative;z-index:1;">

    <!-- Left: Contact info + What to expect -->
    <div>
      <div style="margin-bottom:2rem;">
        <span style="display:inline-flex;align-items:center;gap:.6rem;padding:.4rem 1rem;border-radius:999px;font-size:.75rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#C4B5FD;background:rgba(196,181,253,.08);border:1px solid rgba(196,181,253,.2);margin-bottom:1.5rem;">How to reach us</span>
        <h2 style="font-family:var(--font-display);font-size:clamp(1.5rem,3vw,2rem);font-weight:700;color:#fff;margin:0 0 2rem;line-height:1.2;">We are real people,<br>ready to help</h2>
      </div>
      <div style="display:flex;flex-direction:column;gap:1.25rem;margin-bottom:2.5rem;">
        <?php foreach([
          ['i'=>'fa-phone','c'=>'#60A5FA','bg'=>'rgba(96,165,250,.1)','t'=>'Phone','v'=>PHONE_DISPLAY,'href'=>'tel:'.PHONE_TEL],
          ['i'=>'fa-envelope','c'=>'#A78BFA','bg'=>'rgba(167,139,250,.1)','t'=>'Email','v'=>EMAIL_INFO,'href'=>'mailto:'.EMAIL_INFO],
          ['i'=>'fa-location-dot','c'=>'#6EE7B7','bg'=>'rgba(110,231,183,.1)','t'=>'Mailing Address','v'=>'Brocus IT Solutions LLC<br>'.ADDR_LINE1.'<br>'.ADDR_CITY.', '.ADDR_STATE.' '.ADDR_ZIP,'href'=>null],
          ['i'=>'fa-clock','c'=>'#FCD34D','bg'=>'rgba(252,211,77,.1)','t'=>'Hours','v'=>'Monday to Saturday, '.HOURS_WEEKDAY.'<br>We aim to respond to every message within one business day.','href'=>null],
        ] as $c): ?>
        <div style="display:flex;align-items:flex-start;gap:1.25rem;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:20px;padding:1.5rem 1.75rem;transition:border-color .3s;" onmouseover="this.style.borderColor='rgba(255,255,255,.14)'" onmouseout="this.style.borderColor='rgba(255,255,255,.07)'">
          <div style="width:44px;height:44px;border-radius:14px;background:<?= $c['bg'] ?>;color:<?= $c['c'] ?>;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:1rem;"><i class="fas <?= $c['i'] ?>"></i></div>
          <div>
            <div style="font-size:.72rem;font-weight:700;color:rgba(255,255,255,.35);letter-spacing:.1em;text-transform:uppercase;margin-bottom:.375rem;"><?= $c['t'] ?></div>
            <?php if($c['href']): ?><a href="<?= $c['href'] ?>" style="color:#fff;font-weight:600;text-decoration:none;transition:color .2s;line-height:1.5;" onmouseover="this.style.color='<?= $c['c'] ?>'" onmouseout="this.style.color='#fff'"><?= $c['v'] ?></a>
            <?php else: ?><div style="color:rgba(255,255,255,.75);line-height:1.65;font-size:.92rem;"><?= $c['v'] ?></div><?php endif; ?>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
